...
const Artist = ({ artist }: Props) => {
  return (
    <div
      className="group-one flex w-48 flex-col items-center justify-between gap-3 rounded-xl bg-white p-4 pb-4 text-sm text-slate-700  transition-all duration-300 ease-in-out hover:cursor-pointer hover:bg-slate-100 active:ring-slate-200 dark:bg-slate-900 dark:text-white dark:hover:bg-slate-800 dark:active:ring-1 dark:active:ring-slate-700"
    >
          ...
          <div className="group relative flex items-center justify-center overflow-hidden rounded-md transition-all">
            <img
              ...
              className="h-40 w-40 rounded-xl"
                src={artist?.ImageTags?.Primary ? `https://revised-tribunal-telephony-arise.trycloudflare.com/Items/${artist?.Id}/Images/Primary?maxHeight=400&tag=${artist?.ImageTags?.Primary}&quality=90` : "/images/artist.png"}
              alt=""
            />
          </div>
        </Tilt>
